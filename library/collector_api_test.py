...
class collector_api():
    ...
    # 틱(1분 별) 데이터를 가져오는 함수
    def set_tick_crawler_table(self, code, code_name, date):
        df = self.open_api.get_total_data_tick(code, code_name)

        df_temp = DataFrame(df,
                            columns=['date', 'check_item', 'code', 'code_name', 'd1_diff_rate', 'close', 'open',
                                     'high',
                                     'low',
