...
class open_api(QAxWidget):
    ...
    def send_condition(self, screen_no, condition_name, condition_index, search_type):
        logger.debug("send_condition!! : " + str(condition_name) + " " + str(condition_index))
    ...
    def _receive_tr_condition(self, screen_no, code_list, condition_name, condition_index, next):
        ...
        for code in temp_universe_list:
            if code not in current_universe_list:
                ...
                new_universe_list.append(code)
        # 새로운 조건 검색 결과 종목 코드 리스트로 업데이트
        self.update_db_setting_data_universe_rocket(new_universe_list)
...
############### DB 제어
    ...
    def init_db_setting_data_universe_rocket(self, table_name):
        ...
        df_setting_data_temp = {'code': [],
                                ...
                                'fight_check': []}
        df_setting_data = DataFrame(df_setting_data_temp,
                                    columns=['code', 'init_check', 'fight_check'])

        # 자료형
        df_setting_data.loc[0, 'code'] = str(0)
        df_setting_data.loc[0, 'init_check'] = int(0)
        df_setting_data.loc[0, 'fight_check'] = int(0)

        df_setting_data.to_sql(table_name, self.engine_universe_rocket, if_exists='replace')

        sql = "delete from " + table_name + " where code=0"
        self.engine_universe_rocket.execute(sql)
    ...
    def update_db_setting_data_universe_rocket(self, universe_list):
        ...
        df_setting_data_temp = {'code': universe_list,
                                'init_check': [int(0) for _ in range(len(universe_list))],
                                'fight_check': [int(0) for _ in range(len(universe_list))]}
        df_setting_data = DataFrame(df_setting_data_temp,
                                    columns=['code', 'init_check', 'fight_check'])

        df_setting_data.to_sql(self.universe_list_table_name, self.engine_universe_rocket, if_exists='append')
    ...
    def update_db_universe_rocket(self):
        ...
        df_temp_data = DataFrame(self.temp_data,
                                 columns=['time', 'code', 'price', 'volume'])
    ...
    def get_total_data_tick_for_trade(self, code, date):
        logger.debug("get_total_data_tick_for_trade : " + str(code))
        self.ohlcv = defaultdict(list)
