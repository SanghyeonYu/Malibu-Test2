...
class Data_Loader():
    ...
    def variable_setting(self):
        self.date_list = ["20200824", "20200825", "20200826", "20200827", "20200828",
                          ...
                          "20200921", "20200922", "20200923", "20200924"]
    ...
    def db_name_setting(self):
        self.engine_malibu = create_engine(
            "mysql+mysqldb://" + cf.db_id + ":" + cf.db_passwd + "@" + cf.db_ip + ":" + cf.db_port + "/malibu",
            encoding='utf-8')
        event.listen(self.engine_malibu, 'before_execute', escape_percentage, retval=True)

    # code명으로 code_name을 가져오는 함수
    def get_name_by_code(self, code):
        sql = "select code_name from stock_item_all where code = '%s'"
        code_name = self.engine_malibu.execute(sql % (code)).fetchall()
        # print(code_name)
        if code_name:
    ...
    def candidate_universe_init(self):
        ...
        for date in self.date_list:
            ...
            temp_list = self.engine_malibu.execute(sql).fetchall()
    ...
    def set_day_data(self, date, code):
        ...
        sql = "select close, volume from `" + code_name + "` where date < " + date + " order by date desc limit 120"
        temp_data = self.engine_malibu.execute(sql).fetchall()
